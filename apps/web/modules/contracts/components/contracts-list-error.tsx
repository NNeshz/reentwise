...
export function ContractsListError({ error, onRetry, isRetrying }: Props) {
  const message = errorMessageFromUnknown(error, "No se pudieron cargar los contratos");

  return (
    <Alert variant="destructive" className="max-w-lg">
      <IconAlertCircle className="size-4" aria-hidden />
      <AlertTitle>No se pudieron cargar los contratos</AlertTitle>
